...
class Translator_Model(nn.Module):
    ...
    def pick_animals_buffer(self, picker, qry_img, qry, picker_loader, expansion_size=0, get_img = False, random=False, img_id='', get_original=True, type='image_translator'): 
        if expansion_size == 0:
            get_original = True
        candidate_neighbours = next(iter(picker_loader)) # from train sampler, size: pool_size, 3, h, w + label_size
        nb_images = candidate_neighbours[0].cuda()
        candidate_neighbours = candidate_neighbours[2] # extracts img from img+label
        ...
        if picker is not None:
            ...
            from model.utils import get_sim_scores_orb
            scores = get_sim_scores_orb(qry, candidate_neighbours)
        if random == False:
            scores, idxs = torch.sort(scores, descending=False) # best (lower KL divergence) in front
            idxs = idxs.long()
        ...
        for selected_i in range(expansion_size):
            ...
            if type == 'image_translator' or type == 'random-image_translator':
                translation = self.translate_simple(nb, class_code)
            elif type == 'mix-up' or type == 'random-mix-up' or type == 'sim-mix-up':
                nb = get_orig(nb_name)
                # nb = self.translate_simple(nb, self.compute_k_style(nb, 1))
                translation = 0.5 * (nb + qry)
            translations.append(translation)
