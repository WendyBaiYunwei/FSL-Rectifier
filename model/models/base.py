...
class FewShotModel(nn.Module):
    ...
    def forward(self, x, get_feature=False, qry_expansion=0, spt_expansion=0):
        if get_feature:
            ...
            if spt_expansion > 0:
                new_embs = instance_embs[self.args.way:spt_cutoff].reshape(spt_expansion, self.args.way, -1).mean(0)
                new_spt = 0.5 * (original_embs + new_embs)
            else:
                new_spt = original_embs

            original_embs = instance_embs[spt_cutoff:spt_cutoff+1*self.args.way].reshape(self.args.way, -1)
            if qry_expansion > 0:
                new_embs = instance_embs[spt_cutoff+1*self.args.way:].reshape(qry_expansion, self.args.way, -1).\
                    mean(0)
            ...
            if self.training:
                ...
                # always self.args.way-way-1-shot
