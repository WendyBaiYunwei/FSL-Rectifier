...
from model.FUNIT.utils import get_pretrain_loaders, get_config
...
if __name__ == '__main__':
    ...
    if args.backbone_class == 'Res12':
        config['new_size'] = 92
        config['crop_image_height'] = 84
        config['crop_image_width'] = 84
    train_loader = get_pretrain_loaders(config)
    # args.num_class = trainset.num_class
    ...
    def save_checkpoint(is_best, filename='checkpoint.pth'):
        state = {'epoch': epoch + 1,
                 'args': args,
        ...
        if is_best:
            shutil.copyfile(osp.join(args.save_path, filename), osp.join(args.save_path, 'model_best.pth'))
    
    if args.resume == True:
        # load checkpoint
        state = torch.load(osp.join(args.save_path, 'checkpoint.pth'))
        init_epoch = state['epoch']
        resumed_state = state['params']
